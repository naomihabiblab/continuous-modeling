...
cell_type <- 'oligo'
DATA_PATH <- file.path(get_result_dir(), "oligo_topics")

k <- Sys.getenv(x = 'SLURM_ARRAY_TASK_ID', unset = '8')

obj <- LoadH5Seurat(file.path(SEURAT_OBJECT_DATA_PATH, "oligodendrocytes.h5Seurat"))
obj <- enrich_meta_data(obj)
obj <- fix_similarity_names(obj)


plots_dir <- file.path(get_result_dir(), 'OligoAll')
dir.create(plots_dir, showWarnings = FALSE)

# load fits 
fits <- read_topic_models(file.path(DATA_PATH, "all_oligo_topic_fit"))
...
# DE analysis  ------------------------------------------------------------
print("DE analysis")
dfas_vs_null <- read_de(prefix = file.path(DATA_PATH, "all_oligo_dfa_vsnull"))
