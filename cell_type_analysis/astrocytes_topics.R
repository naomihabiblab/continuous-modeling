...
DATA_PATH <- file.path(get_result_dir(), "astrocytes_topics")

k <- Sys.getenv(x = 'SLURM_ARRAY_TASK_ID', unset = '10')

astrocytes <- LoadH5Seurat(file.path(SEURAT_OBJECT_DATA_PATH, "astrocytes.h5Seurat"))
astrocytes <- enrich_meta_data(astrocytes)
astrocytes <- fix_similarity_names(astrocytes)


plots_dir <- file.path(get_result_dir(), 'AstrocytesAll')
dir.create(plots_dir, showWarnings = FALSE)

# load fits
fits <- read_topic_models(file.path(DATA_PATH, "all_astrocytes_topic_fit"))


print("Adding topics to metadata...")
topcis_and_cols <- add_topics_to_metadata(astrocytes, fits)
astrocytes <- topcis_and_cols$obj
topics_columns_list <- topcis_and_cols$columns_list
rm(topcis_and_cols)
gc()

k <- '10'
save_umap_densities(astrocytes, topics_columns_list[[k]])
...
print("DE analysis")
dfas_vs_null <- read_de(prefix = file.path(DATA_PATH, "all_astrocytes_dfa_vsnull"))
...
df <- df_pos_des(obj=astrocytes, reweighted_f, fits[[k]], assay="SCT", organism='hsa')
...
save_abundance_stat(astrocytes, group = CELL_TYPE_TO_GROUP[[cell_type]], topics_columns=topics_columns_list[[k]])
