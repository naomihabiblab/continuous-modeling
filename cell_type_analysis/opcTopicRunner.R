...
output_dir <- file.path(get_result_dir(), "opcs_topics")
...
output_name_prefix <- "all_opcs"
print(output_name_prefix)

input_path <- file.path(SEURAT_OBJECT_DATA_PATH, "opcs.h5Seurat")
message("Loading data from ", input_path)
obj <- LoadH5Seurat(input_path,
                      assays = list(SCT=c('counts'), RNA=c('counts')),
                      reductions = FALSE,
                      graphs = FALSE,
                      neighbors = FALSE,
                      tools = FALSE,
